...
export type AtlasLane = {
  ...
  y: number;
  height: number;
  tone: "primary" | "depth" | "audit" | "archive";
};
...
const v2Positions: Record<string, AtlasPosition> = {
  ...
  "V2-E-027": { x: 714, y: 78 },
  "V2-E-028": { x: 928, y: 78 },
  "V2-E-029": { x: 1142, y: 78 },
  "V2-E-025": { x: 1356, y: 78 },
  // Lane 2: 352 · REPRESENTASI SENSOR (y: 252–586)
  "V2-E-003": { x: 286, y: 304 },
  "V2-E-004": { x: 500, y: 304 },
  "V2-E-005": { x: 714, y: 304 },
  "V2-E-006": { x: 928, y: 304 },
  "V2-E-008": { x: 928, y: 440 },
  "V2-E-009": { x: 1142, y: 440 },
  "V2-E-010": { x: 1142, y: 304 },
  "V2-E-011": { x: 1356, y: 304 },
  "V2-E-030": { x: 1356, y: 440 },
  "V2-E-031": { x: 1570, y: 440 },
  // Lane 3: DIAGNOSIS & DUA-TAHAP (y: 586–862)
  "V2-E-012": { x: 500, y: 610 },
  "V2-E-013": { x: 714, y: 610 },
  "V2-E-014": { x: 928, y: 610 },
  "V2-E-015": { x: 1142, y: 610 },
  "V2-E-016": { x: 1356, y: 610 },
  "V2-E-017": { x: 1142, y: 746 },
  "V2-E-018": { x: 1356, y: 746 },
  "V2-E-019": { x: 1570, y: 746 },
  "V2-E-020": { x: 1784, y: 746 },
  "V2-E-021": { x: 1998, y: 746 },
  "V2-E-024": { x: 2426, y: 746 },
  "V2-E-026": { x: 2640, y: 746 },
  // Lane 4: SINTESIS, AUDIT & BATAS INFERENSI (y: 862–1072)
  "V2-E-007": { x: 1142, y: 918 },
  "V2-E-032": { x: 1784, y: 918 },
...
export const atlasLanes: AtlasLane[] = [
  {
    id: "rgb-953",
    label: "953 · BENCHMARK & COUNTING",
    caption: "reproduksi, test bersih, dan mono RGB",
    y: 28,
    height: 178,
    tone: "primary",
  },
  {
    id: "depth-352",
    label: "352 · REPRESENTASI SENSOR",
    caption: "RGB, inverse, edge, dan mono depth",
    y: 252,
    height: 334,
    tone: "depth",
  },
  {
    id: "diagnosis",
    label: "DIAGNOSIS & DUA-TAHAP",
    caption: "mekanisme kelas, classifier crop, dan pipeline",
    y: 586,
    height: 276,
    tone: "primary",
  },
  {
    id: "audit",
    label: "SINTESIS, AUDIT & BATAS INFERENSI",
    caption: "power, temporal shift, split, dan sintesis",
    y: 862,
    height: 210,
    tone: "audit",
  },
...
function setRow(
  ...
  ids.forEach((id, index) => {
    positions[id] = {
      x: START_X + (index % columns) * CARD_GAP,
      y: y + Math.floor(index / columns) * 136,
    };
  });
}
...
export function buildAtlasLayout(experiments: Experiment[]) {
  ...
  const dedup = experiments
    .filter(item => item.id.startsWith("HD-"))
    .map(item => item.id);
  const baseline = experiments
    .filter(item => item.id.startsWith("HB-"))
    .map(item => item.id);
  const pipeline = experiments
    .filter(item => item.id.startsWith("RP-E"))
    .map(item => item.id);
...
export function orthogonalPath(
  ...
  const startY =
    ...
    (route.sourceCount === 1
      ? sourceSpan / 2
      : (route.sourceIndex / (route.sourceCount - 1)) * sourceSpan);
  ...
  const targetY =
    ...
    (route.targetCount === 1
      ? targetSpan / 2
      : (route.targetIndex / (route.targetCount - 1)) * targetSpan);
  ...
  const loopY =
    Math.max(parent.y + parentHeight, child.y + childHeight) +
    32 +
    (route.serial % 5) * 18;
